...
class WeckterBackstoryGenerator(toga.App):
    ...
    # Load existing settings if the YAML file exists
                ...
                api_key_exists = settings['api_settings'].get('api_key', None) is not None
    ...
    # Create a text input box for the API key
            ...
            self.api_key_input = toga.PasswordInput(placeholder='Enter OpenAI API Key', style=Pack(padding=5, width=175))
    ...
    # Create a button to save the API key
            save_button = toga.Button('Save', on_press=self.save_api_key, style=Pack(padding=5, width=75))
                    # Create a box to hold the input field and button
            self.api_key_label = toga.Label('OpenAI API Key:', style=Pack(direction=ROW, padding=(10,0,0,0), alignment=CENTER))
            ...
            self.aimodel_selection = toga.Selection(items=["gpt-3.5-turbo", "gpt-4"], on_select=self.on_field_focus_change, style=Pack(padding=(7,10,0,6), width=150))
            self.aimodel_selection.value = self.aimodel

            self.aimodel_label = toga.Label('Select AI Model:', style=Pack(direction=ROW, padding=(13,0,0,2), alignment=CENTER))
            self.aimodel_cost_label = toga.Label('GPT 3.5 ~ $0.002 per Bio\nGPT 4    ~ $0.06  per Bio', style=Pack(direction=ROW, padding=(2,25,0,0), alignment=CENTER))
            self.formatted_total_aimessage_cost = "${:.4f}".format(self.total_aimessage_cost)
            self.token_cost_label = toga.Label('Cost this session: $0.00', style=Pack(direction=ROW, padding=(10,0,0,18) , alignment=CENTER))
    ...
    ###############################################
            ...
            self.rolled_details_TextInput = toga.TextInput(value=self.combined_story, readonly=True, style=Pack(padding=5, width=420))
            ...
            self.cgpt_bio_copy_TextInput = toga.TextInput(value=None, readonly=True, style=Pack(padding=5, width=420))
            ...
            self.cgpt_stat_copy_TextInput = toga.TextInput(value=None, readonly=True, style=Pack(padding=5, width=420))
    ...
    # Create a text input for the D&D character name
            ...
            self.ally_enemy_label = toga.Label('Ally of', style=Pack(direction=ROW, padding=0, alignment=CENTER))
            ...
            self.character_name_input = toga.TextInput(value=None, placeholder='Enter D&D character name', style=Pack(padding=5, width=200))#275
            ...
            self.character_level_input = toga.NumberInput(min_value=1, value=1, step=1, style=Pack(padding=5)) #Remove padding to eliminate Font warning
            ...
            self.character_race_input = toga.TextInput(value=None, placeholder='Enter character race', style=Pack(padding=(0, 10, 0, 35), width=150))
            ...
            self.character_class_input = toga.TextInput(value=None, placeholder='Enter character class', style=Pack(padding=(0, 5, 0, 5), width=150))
    ...
    # Add the "Build" tab and its contents (now wrapped in a ScrollContainer)
            option_container = toga.OptionContainer(
                content=[
                    ("Build", scroll_container),
                    ("Display", display_box),
                    ("ChatGPT Bio", scroll_container_chatgpt),
                    ("ChatGPT Statblock", scroll_container_statblock),
                    ("Settings", scroll_container_appsettings)
                    ]
            )

    # Set the OptionContainer as the main window content
            self.main_window = toga.MainWindow(title=self.formal_name,size=(430, 932))#iPhone 14 Pro Max viewport, actual Pixels 1284px × 2778px
...
####################################################################################################################3
    ...
    #print("sub_categories_and_rolls:", sub_categories_and_rolls)
            ...
            if platform.system() == 'Darwin':
                ...
                self.center_text = 45
            if len(str(category)) < self.center_text: #was set to 45
                diff = self.center_text - len(str(category))
                padding = ' ' * (diff // 2)
